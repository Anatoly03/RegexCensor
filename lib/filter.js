...
import transforms from '../config/transform.js'
...
export function Filter(config) {
    ...
    if (Array.isArray(config.transforms)) {
        for (const transform of transforms) {
            if (
                config.transforms.includes('*') ||
                config.transforms.includes(transform.name)
            ) {
                this.addTransformSet(transform.name)
            }
        }
    }
}
 ...
 * @param {string} key Field Name to add
 */
Filter.prototype.addTransformSet = function (key) {
    if (typeof key != 'string')
        throw 'Filter.addTransform(key) expects string as parameter input'

    for (const field of transforms) {
        if (field.name == key) {
            this.addTransform(field.normalize, field.transforms)
            return
        }
    }
}
...
/**
 * @method addTransform
 * @param {'NFD' | 'NFC'} normalize Field Name to add
 * @param {(string | RegExp)[][]} trans Field Name to add
 */
Filter.prototype.addTransform = function (normalize, trans) {
    for (const field of trans) {
        this.transforms.push([normalize, field[0], field[1]])
    }
}
...
Filter.prototype.checkString = function (str) {
    for (const transform of this.transforms) {
        str = str.normalize(transform[0]).replace(transform[1], transform[2])
    }
